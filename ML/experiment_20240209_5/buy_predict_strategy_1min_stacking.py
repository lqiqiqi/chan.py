...
from ML.experiment_20240209_5.buy_data_generation import buy_stragety_feature
...
def buy_model_predict(code, begin_time, dependent_time, threshold, retrace_rate, wait_time_seconds, end_time=None):
    ...
    model = load(f'buy_model_{code}_bsp3_20240209_5.joblib')
    meta = json.load(open(f"buy_feature_{code}_bsp3_20240209_5.meta", "r"))
    ...
    for last_klu_1m in data_src_1m.get_kl_data():
        ...
        if len(klu_1m_lst_tmp) == 5:  # 已经完成5根1分钟K线了，说明这个最新的5分钟K线和里面的5根1分钟K线在将来不会再变化
            ...
            if is_hold is False and cur_5m_chan[-which_klu_bsp].idx == last_bsp.klu.klc.idx:
            ...
            for retrace_rate in [0.0015]:
                ...
                for wait_time_min in [15 * 60]:
                    ...
                    exp_return, odds, win_rate, average_daily_trades, mean_profit, total_profit, trade_times = buy_model_predict(
                        code=code, begin_time = "2023-12-01 00:01:00", end_time = "2024-01-31 00:00:00", dependent_time = "2023-11-01 00:01:00",
