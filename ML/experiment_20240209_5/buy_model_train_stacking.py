...
def train_buy_model(code, begin_time, end_time):
    X, y = load_svmlight_file(f"buy_feature_{code}_bsp3_20240209_5.libsvm")  # load sample
    # X = X.toarray()
    ...
    clf3 = make_pipeline(
         SimpleImputer(strategy="constant", fill_value=np.nan),
         xgb.XGBClassifier(max_depth=20, n_estimators=20, learning_rate=0.2, objective='binary:logistic',
    ...
    clf4 = make_pipeline(
        SimpleImputer(strategy="constant", fill_value=0),
        BalancedRandomForestClassifier(n_estimators=300, random_state=1, sampling_strategy="all", replacement=True,
                                       bootstrap=False)
    )
    ...
    # dump(sclf, f'buy_model_{code}_bsp3_20240209_5.joblib')
    clf_loaded = load(f'buy_model_{code}_bsp3_20240209_5.joblib')
