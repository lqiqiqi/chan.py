...
def train_buy_model(code,  begin_time, end_time):
    ...
    if code == 'MNQmain':
        param = {'max_depth': 30, 'eta': 0.1, 'objective': 'binary:logistic', 'eval_metric': 'auc',
                 'scale_pos_weight': 1.82, 'reg_lambda': 20}
    elif code == 'MRTYmain':
    ...
    # param = {'objective': 'binary:logistic', 'eval_metric': 'roc_auc'}
    # cv_params = {
    ...
    #     'scale_pos_weight': np.linspace(2, 20, 1)
    # }
    # regress_model = xgb.XGBClassifier(**param)  # 注意这里的两个 * 号！
    # gs = GridSearchCV(regress_model, cv_params, refit=True, verbose=10, cv=5, scoring='roc_auc')
    ...
    preds_binary = (preds > 0.7).astype(int)
