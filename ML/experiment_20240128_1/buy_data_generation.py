    ...
    for bsp_klu_idx, feature_info in bsp_dict.items():
        ...
        for klc in bi_tmp.klc_lst:
            if klc.pre.pre == tmp_klc:
                # 利润要大于 （回撤 + 手续费）
                label = 1 if bi_tmp.get_end_klu().close - klc[-1].close > bi_tmp.get_end_klu().high * 0.001 + (2.8 + 2) / fut_multiplier[code] else 0
                if label == 1:
                    print(tmp_klc.time_end.to_str(), tmp_klc.low, ck_idx_dict[bsp_klu_idx], klc.time_end.to_str(), bi_tmp.get_end_klu().time.to_str(), + (2.8 + 2) / fut_multiplier[code])
