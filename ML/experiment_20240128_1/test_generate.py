...
label_dict = {'MRTYmain': 6, 'MYMmain': 50, 'MNQmain': 26}
...
def train_buy_model(code, begin_time, end_time):
    ...
    for chan_snapshot in chan.step_load():
        ...
        if last_bsp.klu.idx not in bsp_dict and cur_lv_chan[-3].idx == last_bsp.klu.klc.idx and last_bsp.is_buy:
            ...
            #       [(fid, f) for fid, f in bsp_dict[last_bsp.klu.idx]['feature'].items() if fid in ('recent_macd_diverge_before_bsp', 'recent_macd_diverge')])
    ...
    cur_feature_idx = 0
    plot_marker = {}
    fid = open(f"buy_feature_{code}_20240128_1.libsvm", "w")
    for bsp_klu_idx, feature_info in bsp_dict.items():
        ...
        for klc in bi_tmp.klc_lst:
            if klc.pre.pre == tmp_klc:
                label = 1 if bi_tmp.get_end_klu().close - klc[-1].close > bi_tmp.get_end_klu().high * 0.001 * fut_multiplier[code] - 2.8 else 0
                if label == 1:
                    print(tmp_klc.time_end.to_str(), tmp_klc.low, ck_idx_dict[bsp_klu_idx], klc.time_end.to_str(), bi_tmp.get_end_klu().time.to_str(), bi_tmp.get_end_klu().high * 0.001 * fut_multiplier[code] - 2.8)
    ...
    with open(f"buy_feature_{code}_20240128_1.meta", "w") as fid:
        # meta保存下来，实盘预测时特征对齐用
        fid.write(json.dumps(feature_meta))

    X, y = load_svmlight_file(f"buy_feature_{code}_20240128_1.libsvm")  # load sample
    ...
    bst.save_model(f"buy_model_{code}_20240128_1.json")
    ...
    # 全量训练
    dtotal = xgb.DMatrix(f"buy_feature_{code}_20240128_1.libsvm?format=libsvm")  # load sample
    ...
    bst_total.save_model(f"buy_model_{code}_20240128_1.json")
...
if __name__ == '__main__':
    ...
    # "2019-05-20 00:00:00"
    # "2023-12-01 00:00:00"
    train_buy_model(code='MNQmain', begin_time="2024-01-10 00:00:00", end_time="2024-01-18 00:00:00")
