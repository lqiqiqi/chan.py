...
from ML.experiment_20240202_1.buy_data_generation import buy_stragety_feature
...
def buy_model_predict(code, begin_time, only_bsp, is_send, end_time=None):
    """
    ...
    model = xgb.Booster()
    model.load_model(f"buy_model_{code}_20240202_1.json")
    meta = json.load(open(f"buy_feature_{code}_20240202_1.meta", "r"))
    ...
    for chan_snapshot in chan.step_load():
        ...
        if pred_prob > 0.5:
            print(last_bsp.klu.time.to_str(), last_klu.time.to_str(), pred_prob, ' prob gt 0.5')
    ...
    pos_num = (df.prob > 0.5).sum()
    neg_num = (df.prob < 0.5).sum()
    print('正负样本比例：', neg_num / pos_num)
    print('正样本占所有样本的比例：', pos_num / (pos_num + neg_num))
...
if __name__ == '__main__':
    for code in ['MNQmain']:
        ...
        #     buy_model_predict(code=code, begin_time="2022-01-01", only_bsp=False, is_send=True)
        try:
            buy_model_predict(code=code, begin_time=None, end_time='2023-11-03 16:00:00', only_bsp=True, is_send=False)
        except:
            time.sleep(5)
            buy_model_predict(code=code, begin_time="2022-01-01", only_bsp=True, is_send=False)
