        ...
        # 开仓连带固定比例止损单也一起提交；不开仓则继续等
            ...
            if pred_prob > threshold:
                ...
                trail_order_obj = trail_order(
                    account=client_config.account, contract=contract, action='SELL', quantity=1, trailing_percent=trailing_percent)
                trail_oid = trade_client.place_order(trail_order_obj)
                print(f'{cur_lv_chan[-1][-1].time}:trail id = {trail_oid}')
                send_msg(
                    f"美东时间 {now_eastern.strftime('%Y-%m-%d %H:%M:%S')} {code} 程序开仓，最新k线结束时间 {last_klu.time.to_str()}",
                    type='text')
