...
def train_buy_model(code, begin_time, end_time):
    ...
    with open("bsp_dict_20240202_1.pkl", "rb") as f:
    ...
    save_obj(bsp_dict, 'bsp_dict_20240202_1')
    ...
    cur_feature_idx = 0
    plot_marker = {}
    fid = open(f"buy_feature_{code}_20240202_1.libsvm", "w")
    ...
    for bsp_klu_idx, feature_info in bsp_dict.items():
        ...
        for klc in bi_tmp.klc_lst:
            if klc.pre.pre == tmp_klc:
                ...
                label = 1 if bi_tmp.get_end_klu().close - klc[-1].close > bi_tmp.get_end_klu().high * 0.002 + (2.8) / fut_multiplier[code] and klc[-1].close * (1 - 0.002) <= min([klc.low for klc in klc_after_list]) else 0
                if label == 1:
                    print('bsp时间:', tmp_klc.time_end.to_str(), ' bsp低点:', tmp_klc.low, ' bsp收盘:', ck_idx_dict[bsp_klu_idx], ' 买入k线的位置时间:', klc.time_end.to_str(), ' 该笔结束时间:', bi_tmp.get_end_klu().time.to_str(), ' 回撤空间', bi_tmp.get_end_klu().high * 0.002 + (2.8) / fut_multiplier[code], klc[-1].close * (0.002))
    ...
    with open(f"buy_feature_{code}_20240202_1.meta", "w") as fid:
        # meta保存下来，实盘预测时特征对齐用
    ...
    # 全量训练
    dtotal = xgb.DMatrix(f"buy_feature_{code}_20240202_1.libsvm?format=libsvm")  # load sample
    ...
    bst_total.save_model(f"buy_model_{code}_20240202_1.json")
