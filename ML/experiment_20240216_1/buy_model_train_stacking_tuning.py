...
def train_buy_model(code, begin_time, end_time):
    ...
    # X = X.toarray()
    print(np.unique(y, return_counts=True), np.unique(y, return_counts=True)[1][0] / np.unique(y, return_counts=True)[1][1])

    clf3 = make_pipeline(
         SimpleImputer(strategy="constant", fill_value=np.nan),
         xgb.XGBClassifier(objective='binary:logistic',
                             scale_pos_weight=np.unique(y, return_counts=True)[1][0] / np.unique(y, return_counts=True)[1][1], reg_lambda=20, subsample=0.9, random_state=1, missing=np.nan)
    )
    clf4 = make_pipeline(
    ...
    # sclf.get_params()
    params = {'pipeline-1__xgbclassifier__learning_rate': [0.15],
               'pipeline-1__xgbclassifier__n_estimators': [30],
              'pipeline-1__xgbclassifier__max_depth': [30],
              'pipeline-2__balancedrandomforestclassifier__n_estimators': [100, 150, 200]}
    ...
    # 对整个数据集训练
    # dump(sclf, f'buy_model_{code}_ma60_20240216_1.joblib')
    # clf_loaded = load(f'buy_model_{code}_ma60_20240216_1.joblib')
