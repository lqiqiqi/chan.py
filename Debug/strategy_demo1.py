...
if __name__ == "__main__":
    ...
    for chan_snapshot in chan.step_load():  # 每增加一根K线，返回当前静态精算结果
        ...
        if BSP_TYPE.T3A in last_bsp.type or BSP_TYPE.T3B in last_bsp.type:  # 加入只做3类买卖点
        # if BSP_TYPE.T2 in last_bsp.type or BSP_TYPE.T2 in last_bsp.type:  # 加入只做2类买卖点
        #     print(cur_lv_chan[-1][-1].time)
        ...
        last_bsp = bsp_list[-1]
        if is_hold:
            # print('last bsp is sell ', cur_lv_chan[-1][-1].time)
            # print('now time is ', cur_lv_chan[-1][-1].time, ' now price is ', cur_lv_chan[-1][-1].close)
            if cur_lv_chan[-2].fx == FX_TYPE.TOP and last_bsp.is_buy is False and last_bsp.bi.is_sure:  # 顶分型形成后平仓
